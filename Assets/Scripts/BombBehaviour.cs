...
public class BombBehaviour : MonoBehaviour
{
    ...
    [Header("Grid")]
    [SerializeField] WorldGenerator grid;
    ...
    void Explosion(GameObject effect, Vector3 direction)
    {
        ...
        {
            Vector3 explosionPos = transform.position + direction * grid.offset * i;

            Debug.DrawRay(transform.position, explosionPos, Color.red);

            if (Physics.Raycast(transform.position, direction, out RaycastHit hit, grid.offset * i))
            {
